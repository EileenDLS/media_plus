...
import com.darlingweb.jupiter.dao.FavoriteDao;
import com.darlingweb.jupiter.entity.db.Item;
import com.darlingweb.jupiter.entity.db.ItemType;
import com.darlingweb.jupiter.entity.response.Game;
    ...
    private List<Item> recommendByFavoriteHistory (Set<String> favoritedItemIds, List<String> favoritedGameIds, ItemType type)
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            ...
            for (Item item : items) {
                ...
                if (!favoritedItemIds.contains(item.getId())) { // 滤出已经被用户favorite的
                    recommendedItems.add(item);
                }
            }
        }
