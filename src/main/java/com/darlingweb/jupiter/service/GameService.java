...
public class GameService {
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) { //提供game name与否
            return String.format(url, limit);
        } else {
            try {
                // Encode special characters in URL, e.g. Rick Sun -> Rick%20Sun
                // %20 相当于就是一个空格 （+也有这个作用），因为网址中是不可能存在空格的
    ...
    private List<Game> getGameList(String data){
        ...
        try {
            ...
            return Arrays.asList(gameArray);  // array store as list
...
//            item.setGameId(null);
    ...
    public Map<String, List<Item>> searchItems(String gameId) throws TwitchException {
        ...
        for (ItemType type : ItemType.values()) { // 循环三次
